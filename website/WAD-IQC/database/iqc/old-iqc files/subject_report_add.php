...
$add_subject_report_Stmt="Insert into $table_subject_report(category,subject,abreviation,number,grade_ref) values ('%s','%s','%s','%s','%d')";

$delete_Stmt_subject_report = "DELETE from $table_subject_report WHERE
...
if (!mysql_select_db($databaseName, $link)) {
  ...
  if (!(mysql_query(sprintf($delete_Stmt_subject_report,$grade_ref),$link))) 
  {
  ...
  if (!empty($subject))
  {
    ...
    while ($j<sizeof($subject_key)) //loop for $subject
    {
      if ($subject[$subject_key[$j]]=='on')
      {
        ...
        if (!(mysql_query(sprintf($add_subject_report_Stmt,$field_subject->category,$field_subject->subject,
