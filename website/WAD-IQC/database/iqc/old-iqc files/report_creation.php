...
if (!($result_calculation_report=mysql_query(sprintf($calculation_report_Stmt,$grade_ref,$term),$link))) 
{
  DisplayErrMsg(sprintf("Error in executing %s stmt", $subject_Stmt)) ;
  DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
  exit() ;
...
{  
  ...
  $level_first_honor=$field_credits->level_first;
  $level_second_honor=$field_credits->level_second;
  $text_second_honor=$field_credits->text_second;
  $text_first_honor=$field_credits->text_first;
...
if ($k==0)
{
  $credits='';
  $credit_max=5;
  $level_first_honor=80;
  $level_second_honor=90;
  $text_second_honor='2nd Honor';
  $text_first_honor='1st Honor';
  $mark_border=60;
}
...
while (($field_student = mysql_fetch_object($result_student)))
{
      ...
      while($report_counter<=$report_counter_stop)  //start
      {
        ...
        { 
          if ($report_counter<=$term)
          {
            ...
            $report_result->assign("action_term_mark",sprintf("marks_print.php?school=$school&school_year=$school_year&department=$department&grade=$grade&class=$class&subject=$subject_abreviation_array[$i]&term=$report_counter&teacher=$teacher_field&class_ref=$class_ref&v=0&mark_border=$mark_border&t=%d",time()));
  ...
  $honor_name='';
  $bgcolor_honor='white';
  if ($o_a_honor>=$level_second_honor)
  {
    $bgcolor_honor='yellow';
    $honor_name=$text_second_honor;
    if ($o_a_honor>=$level_first_honor)
    {
      $honor_name=$text_first_honor;
    }
  }
  ...
  $report->assign("student_date_of_birth",$student_date_text);
