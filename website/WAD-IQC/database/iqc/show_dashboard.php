...
$selector_Stmt="select * from (select $table_selector.*,g.creation_time,sc.omschrijving as categorie
                from (($table_selector left join selector_categorie sc on $table_selector.selector_categorie_fk=sc.pk)
                inner join $table_selector_user on $table_selector.pk=$table_selector_user.selector_pk
                inner join $table_users on $table_selector_user.user_pk=$table_users.pk)
                inner join $table_gewenste_processen g on g.selector_fk =selector.pk 
                where $table_users.login='$user' and sc.omschrijving!='$omschrijving_dosis_categorie' and g.status='30'
                %s order by $table_selector.name asc, g.pk desc ) temp group by temp.pk";
...
$selector_category_Stmt="select sc.*,g.creation_time
                ...
                left join
                  (select * from gewenste_processen where status=30 order by pk desc) g
...
$selector_modality_Stmt="select modaliteit,max(g.creation_time) as creation_time
                ...
                left join
                  (select * from $table_gewenste_processen where status=30 order by pk desc) g
                on s.pk=g.selector_fk
                where modaliteit > ''
                group by modaliteit
                order by modaliteit";
...
function gewenste_processen($link,$selectorarray) {
   ...
   $query = "select * from
             (select g.pk, s.pk as s_pk
             from selector s
             inner join gewenste_processen g
             on s.pk=g.selector_fk where status='30' and s.pk in ($selectorlist) order by pk desc) temp group by temp.s_pk";
   //printf("%s",$query);exit();
   if (!($result=$link->query($query))) {
      DisplayErrMsg(sprintf("Error in executing %s stmt", $query)) ;
      DisplayErrMsg(sprintf("error: %s", $link->error)) ;
