...
$processor_series_Stmt = "SELECT patient.pat_id AS 'pat_id', patient.pat_name AS 'pat_name', study.accession_no AS 'accession_no', study.study_datetime AS 'study_datetime', selector.name AS 'selector_name', series.modality as 'modality', series.station_name as 'station_name', coalesce(series.pps_start,Max(instance.content_datetime)) as 'series_datetime',gewenste_processen.creation_time AS 'creation_time', status_omschrijving.veld_omschrijving as 'status_omschrijving'
FROM patient
INNER JOIN (
study inner join (
series inner join (
...
group by gewenste_processen.pk,status_omschrijving.veld_omschrijving
ORDER BY 'creation_time'";
