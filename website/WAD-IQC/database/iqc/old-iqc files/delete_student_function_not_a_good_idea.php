...
$student_ref=$student_field->student_ref;

mysql_free_result($result_student);

//selection on all excisting classes
if (!($result_class= mysql_query(sprintf($class_Stmt,$student_ref), $link))) {
DisplayErrMsg(sprintf("Error in executing %s stmt", $subject_Stmt)) ;
DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
exit() ;
}

while($class_field = mysql_fetch_object($result_class);
{

   // by default delete everything except table_department 
    
    //query on subjects
    if (!($result_subject=mysql_query(sprintf($query_subject_Stmt,$class_field->class_ref),$link))) {
    ...
    if (!(mysql_query(sprintf($del_presention_general,$class_field->class_ref), $link))) {
    DisplayErrMsg(sprintf("Error in executing %s stmt", $del_presention_general)) ;
    DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
    exit() ;}
          
    // delete subjects
    if (!(mysql_query(sprintf($del_subjects,$class_field->class_ref), $link))) {
    DisplayErrMsg(sprintf("Error in executing %s stmt", $del_subjects)) ;
    DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
    exit() ;}

    // delete class
    if (!(mysql_query(sprintf($del_class,$class_field->class_ref), $link))) {
    DisplayErrMsg(sprintf("Error in executing %s stmt", $del_klas)) ;
    DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
    exit() ;}

} // all classes and other tables behind that point are deleted.
    ...
    if ($j==0)
    {
      ...
      if ($j==0)
      {
        ...
        if (!($result_school= mysql_query(sprintf($school_Stmt,$student_field->year_ref),$link))) {
        DisplayErrMsg(sprintf("Error in executing %s stmt", $subject_Stmt)) ;
        DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link))) ;
        exit() ;
        }
