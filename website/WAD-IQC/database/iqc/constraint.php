...
if( (!empty($_POST['action'])))
{
  ...
  if ( ($constraint=='Add_Patient')||($constraint=='Modify_Patient') )
  {
    ...
    $addStmt = "Insert into $table_patient(pat_id,pat_id_issuer,pat_name,pat_fn_sx,pat_gn_sx,pat_i_name,pat_p_name,pat_birthdate,pat_sex,pat_custom1,pat_custom2,pat_custom3) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')";
    $updateStmt = "update $table_patient set pat_id='%s',pat_id_issuer='%s',pat_name='%s',pat_fn_sx='%s',pat_gn_sx='%s',pat_i_name='%s',pat_p_name='%s',pat_birthdate='%s',pat_sex='%s',pat_custom1='%s',pat_custom2='%s',pat_custom3='%s' where pk='%s' ";
    
    

    if ($constraint=='Add_Patient')
    {
      $qStmt=sprintf($addStmt,$pat_id,$pat_id_issuer,$pat_name,$pat_fn_sx,$pat_gn_sx,$pat_i_name,$pat_p_name,$pat_birthdate,$pat_sex,$pat_custom1,$pat_custom2,$pat_custom3);
    }
  ...
  if ( ($constraint=='Add_Study')||($constraint=='Modify_Study') )
  {
    ...
    $addStmt = "Insert into $table_study(study_iuid,study_id,study_datetime,accession_no,ref_physician,ref_phys_fn_sx,ref_phys_gn_sx,ref_phys_i_name,ref_phys_p_name,study_desc,study_custom1,study_custom2,study_custom3,study_status_id,mods_in_study,cuids_in_study,num_series,num_instances,ext_retr_aet,retrieve_aets,fileset_iuid,fileset_id,availability,study_status,checked_time,updated_time,created_time) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')";
    $updateStmt = "update $table_study set study_iuid='%s',study_id='%s',study_datetime='%s',accession_no='%s',ref_physician='%s',ref_phys_fn_sx='%s',ref_phys_gn_sx='%s',ref_phys_i_name='%s',ref_phys_p_name='%s',study_desc='%s',study_custom1='%s',study_custom2='%s',study_custom3='%s',study_status_id='%s',mods_in_study='%s',cuids_in_study='%s',num_series='%s',num_instances='%s',ext_retr_aet='%s',retrieve_aets='%s',fileset_iuid='%s',fileset_id='%s',availability='%s',study_status='%s',checked_time='%s',updated_time='%s',created_time='%s' where pk='%s' ";
    
    

    if ($constraint=='Add_Study')
    {
      $qStmt=sprintf($addStmt,$study_iuid,$study_id,$study_datetime,$accession_no,$ref_physician,$ref_phys_fn_sx,$ref_phys_gn_sx,$ref_phys_i_name,$ref_phys_p_name,$study_desc,$study_custom1,$study_custom2,$study_custom3,$study_status_id,$mods_in_study,$cuids_in_study,$num_series,$num_instances,$ext_retr_aet,$retrieve_aets,$fileset_iuid,$fileset_id,$availability,$study_status,$checked_time,$updated_time,$created_time);
    }
  ...
  if ( ($constraint=='Add_Series')||($constraint=='Modify_Series') )
  {
    ...
    {
      $qStmt=sprintf($addStmt,$series_iuid,$series_no,$modality,$body_part,$laterality,$series_desc,$institution,$station_name,$department,$perf_physician,$perf_phys_fn_sx,$perf_phys_gn_sx,$perf_phys_i_name,$perf_phys_p_name,$pps_start,$pps_iuid,$series_custom1,$series_custom2,$series_custom3,$num_instances,$src_aet,$ext_retr_aet,$retrieve_aets,$fileset_iuid,$fileset_id,$availability,$series_status,$created_time,$updated_time,$series_attrs);
    }
 ...
 //Instance
  if ( ($constraint=='Add_Instance')||($constraint=='Modify_Instance') )
  {
    ...
    $addStmt = "Insert into $table_instance(sop_iuid,sop_cuid,inst_no,content_datetime,sr_complete,sr_verified,inst_custom1,inst_custom2,inst_custom3,ext_retr_aet,retrieve_aets,availability,inst_status,all_attrs,commitment,archived,updated_time,created_time,inst_attrs) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')";
    $updateStmt = "update $table_instance set sop_iuid='%s',sop_cuid='%s',inst_no='%s',content_datetime='%s',sr_complete='%s',sr_verified='%s',inst_custom1='%s',inst_custom2='%s',inst_custom3='%s',ext_retr_aet='%s',retrieve_aets='%s',availability='%s',inst_status='%s',all_attrs='%s',commitment='%s',archived='%s',updated_time='%s',created_time='%s',inst_attrs='%s' where pk='%s' ";
   

    if ($constraint=='Add_Instance')
    {
      $qStmt=sprintf($addStmt,$sop_iuid,$sop_cuid,$inst_no,$content_datetime,$sr_complete,$sr_verified,$inst_custom1,$inst_custom2,$inst_custom3,$ext_retr_aet,$retrieve_aets,$availability,$inst_status,$all_attrs,$commitment,$archived,$updated_time,$created_time,$inst_attrs);
    }
...
if ( ($constraint=='Add_Patient')||($constraint=='Modify_Patient') )
{
  ...
  if ($selector_patient_fk<=0)  //new patient
  {
    $table_patient=$patient->fetch("constraint_patient.tpl");
  }

  if ($selector_patient_fk>0)  //update patient
  {
    $table_patient='selector_patient';
...
if ( ($constraint=='Add_Study')||($constraint=='Modify_Study') )
{
  ...
  if ($selector_study_fk<=0)  //new study
  {
    $table_study=$study->fetch("constraint_study.tpl");
  }

  if ($selector_study_fk>0)  //update study
  {
    $table_study='selector_study';
...
if ( ($constraint=='Add_Series')||($constraint=='Modify_Series') )
{
 $series = new Smarty_NM();
  ...
  if ($selector_series_fk<=0)  //new series
  {
    $table_series=$series->fetch("constraint_series.tpl");
  }

  if ($selector_series_fk>0)  //update series
  {
    $table_series='selector_series';
...
if ( ($constraint=='Add_Instance')||($constraint=='Modify_Instance') )
{
 $instance = new Smarty_NM();
  ...
  if ($selector_instance_fk<=0)  //new instance
  {
    $table_instance=$instance->fetch("constraint_instance.tpl");
  }

  if ($selector_instance_fk>0)  //update instance
  {
    $table_instance='selector_instance';
...
$start_self = sprintf("http://%s",$_SERVER['HTTP_HOST']);
$constraint_pssi->assign("action_constraint",sprintf("constraint.php?constraint=$constraint&selector_patient_fk=$selector_patient_fk&selector_study_fk=$selector_study_fk&selector_series_fk=$selector_series_fk&selector_instance_fk=$selector_instance_fk&pk=$pk&t=%d",time()) );
